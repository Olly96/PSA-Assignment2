...
public class Timer {
    ...
    public <T, U> double repeat(int n, Supplier<T> supplier, Function<T, U> function, UnaryOperator<T> preFunction, Consumer<U> postFunction) {
        ...
        /*
         * nullifying ticks to zero and running as false as we are supposed to time only target function and
         * resume has been called from constructor itself so we'll indirectly end up timing 1st preFunction run
         */
        ticks = 0;
        running = false;
        for (int i = 1; i <= n; i++) {
            ...
            resume();
            U foo = function.apply(supplier.get());
            pauseAndLap();
            if(postFunction != null){
                postFunction.accept(foo);
            }
        }
        return meanLapTime();
    }
